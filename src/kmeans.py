...
def divide(data, labels):
    """
    Divide into a list of clusters by labels
    """
...
def within_cluster_dist_sum(cluster, centroid, cluster_id):
    if cluster_id in within_cluster_dist_sum_store:
        ...
        result = (((cluster - centroid) ** 2).sum(axis=1) ** 0.5).sum()
...
def RMSSTD(data, clusters, centroids):
    ...
    return (SSE(clusters, centroids) / (attribute_num * df)) ** 0.5
...
def DB_find_max_j(clusters, centroids, i):
    ...
    for j in range(len(clusters)):
        ...
        val = (cluster_i_stat + cluster_j_stat) / (((centroids[i] - centroids[j]) ** 2).sum() ** 0.5)
        if val > max_val:
            max_val = val
            max_j = j
    return max_val
...
def kmeans_metadata(d, ts):
    ...
    silhouette_score = metrics.silhouette_score(d, labels, metric="euclidean", sample_size=len(d))
    davies_bouldin_score = dbindex(d, labels)
    calinski_harabaz_score = chindex(d, labels)
    adjusted_random_score = adjusted_rand(ts, labels)
    ...
    return (
        ...
        all_score["cohesion"],
        all_score["separation"],
        all_score["RMSSTD"],
        all_score["RS"],
        all_score["XB"],
        adjusted_random_score,
        adjust_mutual,
        fowlkes_score,
        mutual_info_score,
    )
...
class KMeansMetadata:
    def __init__(self) -> None:
        ...
        self.davies_bouldin_score: float
        self.calinski_harabaz_score: float
        self.cohesion_score: float
        self.separation_score: float
        self.rmsstd_score: float
        self.rs_score: float
        self.xb_score: float
        self.adjusted_random_score: float
        self.adjusted_mutual_info_score: float
        self.fowlkes_mallows_score: float
        self.normalized_mutual_info_score: float

    def fit(self, data, target):
        (
            self.silhouette_score,
            self.davies_bouldin_score,
            self.calinski_harabaz_score,
            self.cohesion_score,
            self.separation_score,
            self.rmsstd_score,
            self.rs_score,
            self.xb_score,
            self.adjusted_random_score,
            self.adjusted_mutual_info_score,
            self.fowlkes_mallows_score,
            self.normalized_mutual_info_score,
        ) = kmeans_metadata(data, target)
        return self