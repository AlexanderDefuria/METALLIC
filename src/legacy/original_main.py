...
import weighted_complexity
import dual_weighted_complexity
import tomeklink_complexity
...
p = Path(__file__).parent / "features.csv"
l = path.exists(p)
fields = [
    "Dataset",
    "Original Rows",
    ...
    "total_hypersphere",
    "hypersphere_minority",
    "hypersphere_majority",
    "samplesperhypersphere",
    "samplesperhypersphere_minority",
    "samplesperhypersphere_majority",
    "Average distance between class",
    "overlapping",
    "ComplexityMetric",
    "WeightedCM",
    "DualWeightedCM",
...
files = sorted(glob.glob(str(dataset_dir) + "/*.csv"))
train_time = []
predict_time = []
for file_new in files:
    ...
    # print(data.Class)
    complex_metric = complexity_metric.complexity(data)
    weighted_complex_metric = weighted_complexity.weighted_complexity(data)
    dualweight_complex_metric = dual_weighted_complexity.dualweighted_complexity(data)
    tomelink_complex_metric = tomeklink_complexity.tomelink_complexity(data)
    ...
    minority_hypersphere = [
        minority_class,
        groupsPerClass[minority_class_index],
    ]  # groupsPerClass[minority_class_index] will give you minority class hypersphere
    ...
    cv_splits = 3
    cross_validation = StratifiedKFold(n_splits=cv_splits, shuffle=True)  # cross validation to 3
    ...
    for i in [
        ...
        for i, (train_index, test_index) in enumerate(cross_validation.split(uX, uy)):
            ...
            # print(len(y_test))
            # knn
            f1_score_knn, accuracy_knn, geometric_mean_knn, precision_knn, recall_knn, auc_roc_knn, auc_pr_knn, bal_acc_knn, cwascore_knn = (
                knn.knn_classifier(X_train, y_train, X_test, y_test, state)
            )
            ...
            # decision tree
            f1_score_dt, accuracy_dt, geometric_mean_dt, precision_dt, recall_dt, auc_roc_dt, auc_pr_dt, bal_acc_dt, cwascore_dt = (
                decision_tree.decision_tree(X_train, y_train, X_test, y_test, state)
            )
            ...
            # GaussianNB
            f1_score_GNB, accuracy_GNB, geometric_mean_GNB, precision_GNB, recall_GNB, auc_roc_GNB, auc_pr_GNB, bal_acc_GNB, cwascore_GNB = (
                gaussian.Gaussian(X_train, y_train, X_test, y_test, state)
            )
            ...
            # print(i)
            f1_score_SVM, accuracy_SVM, geometric_mean_SVM, precision_SVM, recall_SVM, auc_roc_SVM, auc_pr_SVM, bal_acc_SVM, cwascore_SVM = (
                SVM.SVM_classifier(X_train, y_train, X_test, y_test, state)
            )
            ...
            # Random forest
            f1_score_RF, accuracy_RF, geometric_mean_RF, precision_RF, recall_RF, auc_roc_RF, auc_pr_RF, bal_acc_RF, cwascore_RF = (
                random_forest.rf_classifier(X_train, y_train, X_test, y_test, state)
            )
            ...
            # xgboost
            f1_score_XGB, accuracy_XGB, geometric_mean_XGB, precision_XGB, recall_XGB, auc_roc_XGB, auc_pr_XGB, bal_acc_XGB, cwascore_XGB = (
                xgb.XGB_classifier(X_train, y_train, X_test, y_test, state)
            )
            ...
            # adaboost
            f1_score_ADA, accuracy_ADA, geometric_mean_ADA, precision_ADA, recall_ADA, auc_roc_ADA, auc_pr_ADA, bal_acc_ADA, cwascore_ADA = (
                adaboost.ADA_classifier(X_train, y_train, X_test, y_test, state)
            )
            ...
            # catboost
            f1_score_CAT, accuracy_CAT, geometric_mean_CAT, precision_CAT, recall_CAT, auc_roc_CAT, auc_pr_CAT, bal_acc_CAT, cwascore_CAT = (
                cat.CAT_classifier(X_train, y_train, X_test, y_test, state)
            )
        ...
        # knn
        avg_f1_score_knn = sum(f1_knn) / cv_splits
        avg_acc_knn = sum(acc_knn) / cv_splits
        avg_gmean_knn = sum(gmean_knn) / cv_splits
        avg_precision_knn = sum(preci_knn) / cv_splits
        avg_recall_knn = sum(rec_knn) / cv_splits
        avg_roc_knn = sum(roc_knn) / cv_splits
        avg_pr_knn = sum(pr_knn) / cv_splits
        avg_bal_knn = sum(bal_knn) / cv_splits
        avg_cwa_knn = sum(cwa_knn) / cv_splits
        # decision tree
        avg_f1_score_dt = sum(f1_dt) / cv_splits
        avg_acc_dt = sum(acc_dt) / cv_splits
        avg_gmean_dt = sum(gmean_dt) / cv_splits
        avg_precision_dt = sum(preci_dt) / cv_splits
        avg_recall_dt = sum(rec_dt) / cv_splits
        avg_roc_dt = sum(roc_dt) / cv_splits
        avg_pr_dt = sum(pr_dt) / cv_splits
        avg_bal_dt = sum(bal_dt) / cv_splits
        avg_cwa_dt = sum(cwa_dt) / cv_splits

        # GaussianNB
        avg_f1_score_GNB = sum(f1_GNB) / cv_splits
        avg_acc_GNB = sum(acc_GNB) / cv_splits
        avg_gmean_GNB = sum(gmean_GNB) / cv_splits
        avg_precision_GNB = sum(preci_GNB) / cv_splits
        avg_recall_GNB = sum(rec_GNB) / cv_splits
        avg_roc_GNB = sum(roc_GNB) / cv_splits
        avg_pr_GNB = sum(pr_GNB) / cv_splits
        avg_bal_GNB = sum(bal_GNB) / cv_splits
        avg_cwa_GNB = sum(cwa_GNB) / cv_splits
        # SVM
        avg_f1_score_SVM = sum(f1_SVM) / cv_splits
        avg_acc_SVM = sum(acc_SVM) / cv_splits
        avg_gmean_SVM = sum(gmean_SVM) / cv_splits
        avg_precision_SVM = sum(preci_SVM) / cv_splits
        avg_recall_SVM = sum(rec_SVM) / cv_splits
        avg_roc_SVM = sum(roc_SVM) / cv_splits
        avg_pr_SVM = sum(pr_SVM) / cv_splits
        avg_bal_SVM = sum(bal_SVM) / cv_splits
        avg_cwa_SVM = sum(cwa_SVM) / cv_splits
        # RF
        avg_f1_score_RF = sum(f1_RF) / cv_splits
        avg_acc_RF = sum(acc_RF) / cv_splits
        avg_gmean_RF = sum(gmean_RF) / cv_splits
        avg_precision_RF = sum(preci_RF) / cv_splits
        avg_recall_RF = sum(rec_RF) / cv_splits
        avg_roc_RF = sum(roc_RF) / cv_splits
        avg_pr_RF = sum(pr_RF) / cv_splits
        avg_bal_RF = sum(bal_RF) / cv_splits
        avg_cwa_RF = sum(cwa_RF) / cv_splits

        # XGB
        avg_f1_score_XGB = sum(f1_XGB) / cv_splits
        avg_acc_XGB = sum(acc_XGB) / cv_splits
        avg_gmean_XGB = sum(gmean_XGB) / cv_splits
        avg_precision_XGB = sum(preci_XGB) / cv_splits
        avg_recall_XGB = sum(rec_XGB) / cv_splits
        avg_roc_XGB = sum(roc_XGB) / cv_splits
        avg_pr_XGB = sum(pr_XGB) / cv_splits
        avg_bal_XGB = sum(bal_XGB) / cv_splits
        avg_cwa_XGB = sum(cwa_XGB) / cv_splits

        # ADA
        avg_f1_score_ADA = sum(f1_ADA) / cv_splits
        avg_acc_ADA = sum(acc_ADA) / cv_splits
        avg_gmean_ADA = sum(gmean_ADA) / cv_splits
        avg_precision_ADA = sum(preci_ADA) / cv_splits
        avg_recall_ADA = sum(rec_ADA) / cv_splits
        avg_roc_ADA = sum(roc_ADA) / cv_splits
        avg_pr_ADA = sum(pr_ADA) / cv_splits
        avg_bal_ADA = sum(bal_ADA) / cv_splits
        avg_cwa_ADA = sum(cwa_ADA) / cv_splits

        # CAT
        avg_f1_score_CAT = sum(f1_CAT) / cv_splits
        avg_acc_CAT = sum(acc_CAT) / cv_splits
        avg_gmean_CAT = sum(gmean_CAT) / cv_splits
        avg_precision_CAT = sum(preci_CAT) / cv_splits
        avg_recall_CAT = sum(rec_CAT) / cv_splits
        avg_roc_CAT = sum(roc_CAT) / cv_splits
        avg_pr_CAT = sum(pr_CAT) / cv_splits
        avg_bal_CAT = sum(bal_CAT) / cv_splits
        avg_cwa_CAT = sum(cwa_CAT) / cv_splits

        csvwriter.writerow(
            [
                new_file_name,
                no_of_rows_original,
                no_of_columns_original,
                state_value,
                Silhouettescore,
                DaviesBouldinscore,
                Calinskiharabazscore,
                ...
                over,
                complex_metric,
                weighted_complex_metric,
                dualweight_complex_metric,
                tomelink_complex_metric,
                None_value,
                SMOTE_value,
                NearMiss_value,
                SMOTEENN_value,
                Randomoversampling_value,
                ADASYN_value,
                BorderlineSMOTE_value,
                SVMSMOTE_value,
                RandomUnderSampler_value,
                ClusterCentroids_value,
                NearMissversion1_value,
                NearMissversion2_value,
                NearMissversion3_value,
                TomekLinks_value,
                EditedNearestNeighbours_value,
                RepeatedEditedNearestNeighbours_value,
                AllKNN_value,
                CondensedNearestNeighbour_value,
                NeighbourhoodCleaningRule_value,
                InstanceHardnessThreshold_value,
                SMOTETomek_value,
                1,
                0,
                0,
